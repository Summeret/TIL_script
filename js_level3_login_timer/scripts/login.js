...
let userLogin = true; //사용자의 로그인 상태(참,거짓)에 따라 실행결정
...
function logoutTimerFunc(){
    clearInterval(logoutInterval); //이전에 반복하던 setInterval을 멈춘다 -> 밑에 변수를 위에서 미리 선언하기

    logoutInterval = setInterval(()=>{
        console.log('활동없음. 자동로그아웃합니다.');
        window.location.href = './index.html';
    },logoutTime)
}

//사용자의 활동감지 이벤트(마우스이동, 키보드 누를때, 마우스클릭)
